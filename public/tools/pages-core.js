...
<section><div class="wrap">
  <div class="grid g2">
    ${CARS.map((c, i) => `<div class="card car-card${c.status === 'soon' ? ' soon' : ''}">
      <div class="ph">${c.status === 'live' ? `<img src="${PHOTOS.gallery[0].src}" alt="${esc(c.name)} rental in Boise" loading="lazy" onerror="this.style.display='none'">` : (c.photo ? `<img src="${c.photo}" alt="${esc(c.name)}, coming soon to Boise Sports &amp; Luxury Car Rentals" loading="lazy" onerror="this.style.display='none'">` : '')}</div>
